...
def write_readme(manifest):
    o = ["# Zoley website - v2 sections\n",
         ...
         f'```html\n<script src="https://cdn.jsdelivr.net/gh/ZoleyZoley/zoley-website@{CURRENT_TAG}/dist/zoley-loader.js"></script>\n```\n',
         ...
         f"**To ship a change:** edit the section -> `git push` -> `git tag {NEXT_TAG} && git push --tags` ->",
         f"change `@{CURRENT_TAG}` to `@{NEXT_TAG}` in the header snippet. That number is the only thing you edit in Squarespace.\n",
    ...
    for folder in ORDER:
        ...
        o.append(f'\nWhole page: `<div data-zoley-page="{folder}"></div>`')
